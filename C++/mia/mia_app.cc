...
void MiaApp::InitNanoGUI() {
  ...
  b->setCallback([this] {
    ...
    if (fname != "") {
      i_editor.LoadFromFile(fname);
      ResizeWindow(i_editor.pixel_buffer()->width(),
                   i_editor.pixel_buffer()->height());
    }
  });
  ...
  b->setCallback([this] { i_editor.ApplyBlurFilter(blur_radius_); });
  ...
  b->setFixedSize({150, 30});
  b->setCallback([this] { i_editor.ApplySharpenFilter(sharpen_radius_); });
  ...
  b->setFixedSize({150, 30});
  b->setCallback([this] { i_editor.ApplyEdgeDetectFilter(); });
  ...
  b->setFixedSize({150, 30});
  b->setCallback(
      [this] { i_editor.ApplyThresholdFilter(thresh_cutoff_); });
  ...
  b->setFixedSize({150, 30});
  b->setCallback([this] {
    i_editor.ApplyQuantizeFilter(static_cast<int>(quant_bins_));
  });
...
void MiaApp::InitOpenGL() {
  display_texture_.InitFromFloats(i_editor.pixel_buffer()->width(),
                                  i_editor.pixel_buffer()->height(),
                                  i_editor.pixel_buffer()->data());
}
...
void MiaApp::DrawUsingOpenGL() {
  undo_btn_->setEnabled(i_editor.can_undo());
  redo_btn_->setEnabled(i_editor.can_redo());

  if (display_texture_.initialized()) {
    display_texture_.UpdateFromFloats(i_editor.pixel_buffer()->data());
    ColorData bg = i_editor.pixel_buffer()->background_color();
...
void MiaApp::OnLeftMouseDown(const mingfx::Point2 &pos) {
  tool_x_ = pos[0];
  tool_y_ = pos[1];
  i_editor.StartStroke(tool_name_, tool_color_, tool_radius_, tool_x_,
                            tool_y_);
  painting_ = true;
}
...
void MiaApp::OnLeftMouseUp(const mingfx::Point2 &pos) {
  tool_x_ = pos[0];
  tool_y_ = pos[1];
  i_editor.EndStroke(pos[0], pos[1]);
  painting_ = false;
}
...
void MiaApp::UpdateSimulation(double dt) {
  (void)dt;
  if (painting_) {
    i_editor.AddToStroke(tool_x_, tool_y_);
  }
}

void MiaApp::OnWindowResize(int new_width, int new_height) {
  i_editor.pixel_buffer()->Resize(new_width, new_height);
...
Tool *MiaApp::GetToolByName(const std::string &name) {
  return i_editor.GetToolByName(name);
}

void MiaApp::StartStroke(const std::string &tool_name,
                                const ColorData &color, float radius, int x,
                                int y) {
  i_editor.StartStroke(tool_name, color, radius, x, y);
}

void MiaApp::AddToStroke(int x, int y) {
  i_editor.AddToStroke(x, y);
}

void MiaApp::EndStroke(int x, int y) {
  i_editor.EndStroke(x, y);
}

void MiaApp::LoadFromFile(const std::string &filename) {
  i_editor.LoadFromFile(filename);
}

void MiaApp::SaveToFile(const std::string &filename) {
  i_editor.SaveToFile(filename);
}
...
void MiaApp::ApplyBlurFilter(float radius) {
  i_editor.ApplyBlurFilter(radius);
}

void MiaApp::ApplyMotionBlurFilter(float rad,
        ImageEditor::MBlurDir dir) {
    i_editor.ApplyMotionBlurFilter(rad, dir);
}

void MiaApp::ApplySharpenFilter(float rad) {
  i_editor.ApplySharpenFilter(rad);
}
...
void MiaApp::ApplyThresholdFilter(float value) {
  i_editor.ApplyThresholdFilter(value);
}

void MiaApp::ApplySaturateFilter(float scale) {
    i_editor.ApplySaturateFilter(scale);
}

void MiaApp::ApplyChannelsFilter(float red, float green, float blue) {
    i_editor.ApplyChannelsFilter(red, green, blue);
}

void MiaApp::ApplyQuantizeFilter(int num) {
  i_editor.ApplyQuantizeFilter(num);
}

bool MiaApp::can_undo() { return i_editor.can_undo(); }

bool MiaApp::can_redo() { return i_editor.can_redo(); }

void MiaApp::Undo() { i_editor.Undo(); }

void MiaApp::Redo() { i_editor.Redo(); }

void MiaApp::SaveStateForPossibleUndo() {
  i_editor.SaveStateForPossibleUndo();
}

PixelBuffer *MiaApp::pixel_buffer() { return i_editor.pixel_buffer(); }

void MiaApp::set_pixel_buffer(PixelBuffer *buffer) {
  i_editor.set_pixel_buffer(buffer);
}
