...
##### MY LOG #####
# changed softmax_cross_entropy_with_logits function calls to softmax_cross_entropy_with_logits_v2
# changed tf.initialize_all_variables() to tf.initializers.global_variables() in line 361

MODE =  'acwgan' #'lsgan'# dcgan, , wgan, wgan-gp,
...
PREITERATIONS = 2000 #Number of preiteration training cycles to run OVER NINE THOUSAND!!!
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    session.run(tf.initializers.global_variables(), feed_dict={generated_labels_conv: genRandomLabels(BATCH_SIZE,CLASSES)})
    gen = train_gen()

    saver = tf.train.Saver()

    checkpoint_path = os.path.normpath("/Users/A-dizzle/PycharmProjects/GANs/GANGogh/" + MODE + ".ckpt")
    # save_path = saver.save(session, checkpoint_path)
    # print("saved path at " + checkpoint_path + '.index')
    loaded = False
    if os.path.isfile(checkpoint_path + '.index'):
        print("Restoring session from " + checkpoint_path + "..." )
    ...
    if not loaded:
        for iterp in range(PREITERATIONS):
            ...
            if iterp % 100 == 99:
                print('iteration ' + str(iterp) + ' pretraining accuracy: ' + str(accuracy))
    ...
    for iteration in range(ITERS):
        ...
        if (iteration % 100 == 99 and iteration<1000) or iteration % 1000 == 999 :
            ...
            lib.plot.plot('dev disc cost', np.mean(dev_disc_costs)) # cost of prediction on fake data minus prediction on real data for source
            lib.plot.plot('wgan dev disc cost', _dev_disc_cost_test) # idk
            lib.plot.plot('dev class cost', _class_cost_test) # discriminators cost of class prediction on real data
            lib.plot.plot('dev generated class cost', _gen_class_cost) # generator cost. cost of what generator was instructed to make vs what the discriminator labeled it as
            lib.plot.plot('dev gen cost', _dev_gen_cost_test) # the cost of discriminators prediction of generated data, real vs fake
            lib.plot.plot('dev gen accuracy', _dev_genAccuracy) # discriminator class prediction tested on generated data
            lib.plot.plot('dev real accuracy', _dev_realAccuracy) # discriminator class prediction tested on real data


        if iteration % 1000 == 999:
            generate_image(iteration)
            #Can add generate_good_images method in here if desired
            # checkpoint_path = "/Users/A-dizzle/PycharmProjects/GANs/GANGogh/misc/model.ckpt"
            # cp_callback = tf.keras.callbacks.ModelCheckpoint(checkpoint_path,
            #                                                  save_weights_only=True,
            #                                                  verbose=1)
            # print("Model saved in path: " + checkpoint_path)
            save_path = saver.save(session, checkpoint_path)
            print("saved state at iteration " , iteration, "at file " + checkpoint_path)
