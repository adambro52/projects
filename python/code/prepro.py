            ...
            for para in article["paragraphs"]:
                ...
                for qa in para["qas"]:
                    ...
                    for answer in qa["answers"]:
                        ...
                        answer_span = []
                        # if qa['is_impossible']: # does not seem to catch empty y1s and y2s
                        #     pdb.set_trace()
                        for idx, span in enumerate(spans): # gets the indicies of the words of the answers
                            if not (answer_end <= span[0] or answer_start >= span[1]):
                                # pdb.set_trace()
                                answer_span.append(idx)
                        y1, y2 = answer_span[0], answer_span[-1]
                        y1s.append(y1)  # ys are empty and this is stopping the program
                        y2s.append(y2)  # they should be the start and end index of answers, respectively
                                        # i.e. the set of answers
                    # if not (y1s or y2s):
                    #     pdb.set_trace()
...
def build_features(config, examples, data_type, out_file, word2idx_dict, char2idx_dict, is_test=False):
    ...
    def filter_func(example, is_test=False): # currently skips when no answer is found i.e. y1s and y2s are empty
                                             # look ito if that is good
        # pdb.set_trace()
        if not (example["y2s"] and example["y1s"]):
            return True
        return len(example["context_tokens"]) > para_limit or \
               len(example["ques_tokens"]) > ques_limit or \
               (example["y2s"][0] - example["y1s"][0]) > ans_limit # both y1s and y2s are empty on iteration 6
